...
public class JwtUtil {
    ...
    public String createJwt(LoginManager loginManager){
        ...
        String token =  JWT.create()
                            ...
                            .withClaim("managerId",loginManager.getManager().getManagerId())
                            .withClaim("managerName", loginManager.getUsername())
                            .withExpiresAt(expireTime())
                            .withIssuedAt(new Date())
                            .sign(algorithm);
        // 存储 token 到 Redis

        stringRedisTemplate.opsForValue().set("JWT_TOKEN:" + loginManager.getManager().getManagerName(), token, expireTime().getTime() - System.currentTimeMillis(), TimeUnit.MILLISECONDS);
        return token;
    }
...
//
    ...
    public LoginManager toManager(DecodedJWT jwt){
        ...
        Manager manager = managerMapper.selectById(claims.get("managerId").toString());
